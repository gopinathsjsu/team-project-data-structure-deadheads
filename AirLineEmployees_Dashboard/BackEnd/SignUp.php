...
<body>
  ...
  <!-- title of website -->
  <div class="Site-Title"><a href="../index.html" class="Text">DESTINATION EXPLORER</a></div>
  ...
  if (isset($_POST['signup'])) {
    ...
    if ($emailres->num_rows > 0) {
      ...
      if ($password === $cpassword) {
        $online="online";
        $location="india";
        $sql = "INSERT INTO users(name, email, username, password, status,location) VALUES('$name', '$email', '$username', '$pass', '$online','$location')";
        ...
        if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
            $id = $row['user_id'];
            $sql = "INSERT into notifications(user_id, notification) VALUES('$id', 'Welcome to DestinationExplorer')";
            $res = $conn->query($sql);
          }
        }
